...
def lambda_handler(event, context):
    try:
        ...
        # not round 1
            ...
            for item in membersItems:
                size_prev_match = get_size_prev_match(
                    item, roundsItems[1], int(lastKey)
                )
                possible_matches, num_possible_matches = get_possible_matches(df, item)
                data.append(
                    {
            ...
            "body": json.dumps(
                {
                    "statusCode": 200,
                    "message": "Success!",
                    "data": {"round": lastKey, "status": True},
                }
            ),
        }
...
def get_possible_matches(df, member):
    ...
    non_previous_matches = df.index.to_numpy()[
        ~np.in1d(df.index.to_numpy(), member["prior_matches"])
    ]
    non_same_department = df.index.to_numpy()[
        ~(member["department"] == df["department"])
    ]
...
def perform_random_loop(df, number4groups):
    ...
    df.loc[:, "randint"] = np.random.choice(
        np.arange(0, len(df)), size=len(df), replace=False
    )
    ...
    for i, (index, row) in enumerate(
        df.sort_values(
            ["size_prev_match", "num_possible_matches", "randint"]
        ).iterrows()
    ):
        ...
        if i == 0:
            ...
            if len(remaining) == 0:
                ...
                p1_possible = np.intersect1d(
                    remaining.loc[index, "possible_matches"], remaining.index.tolist()
                )
        ...
        p1p2_possible_step1 = np.array(p1_possible)[
            ~np.isin(p1_possible, p2.index.tolist())
        ]  # remove p2
        ...
        if i < number4groups:
            ...
            p1p2p3_possible_step1 = np.array(p1p2_possible)[
                ~np.isin(p1p2_possible, p3.index.tolist())
            ]
            # keep only person3's possible matches
            p1p2p3_possible = p1p2p3_possible_step1[
                np.isin(p1p2p3_possible_step1, p3_possible)
            ]
            ...
            df.loc[index, "current_match"].extend(
                [index, p2.index[0], p3.index[0], p4.index[0]]
            )
            df.loc[p2.index[0], "current_match"].extend(
                [p2.index[0], index, p3.index[0], p4.index[0]]
            )
            df.loc[p3.index[0], "current_match"].extend(
                [p3.index[0], index, p2.index[0], p4.index[0]]
            )
            df.loc[p4.index[0], "current_match"].extend(
                [p4.index[0], index, p2.index[0], p3.index[0]]
            )
            ...
            df.loc[p2.index[0], "current_match"].extend(
                [p2.index[0], index, p3.index[0]]
            )
            df.loc[p3.index[0], "current_match"].extend(
                [p3.index[0], index, p2.index[0]]
            )
        ...
        if i == 0:
            if i < number4groups:
                remaining = df.loc[
                    df.index.difference((index, p2.index[0], p3.index[0], p4.index[0]))
                ]
            else:
                remaining = df.loc[
                    df.index.difference((index, p2.index[0], p3.index[0]))
                ]
        if i > 0:
            if i < number4groups:
                remaining = remaining.loc[
                    remaining.index.difference(
                        (index, p2.index[0], p3.index[0], p4.index[0])
                    )
                ]
            else:
                remaining = remaining.loc[
                    remaining.index.difference((index, p2.index[0], p3.index[0]))
                ]
